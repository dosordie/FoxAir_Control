# -*- coding: utf-8 -*-
"""Standard-Modbus Worker-Hilfslogik fuer FoxAir / Phnix Control.

Fix33 Refactor 3:
Der Standard-Modbus-Pfad bekommt einen eigenen Init-Controller. Er verwendet
weiterhin den normalen Haupt-ReaderWorker des Hauptfensters, ist aber logisch
vom Warmlink/LTE-Controller getrennt. Damit bleiben Warmlink-spezifische
Logmeldungen und spaetere Sonderbehandlungen aus dem Standard-Modbus-Pfad raus.
"""
...
STANDARD_MODBUS_INIT_BLOCKS: list[tuple[int, int, str]] = [
    (1001, 90, "Standard Init Extra Paketkopf/Block 1001/0x03E9"),
    (1018, 73, "Standard Init V1.3 Paket 1 Nutzdaten 1018/0x03FA"),
    (1091, 90, "Standard Init Extra Paketkopf/Block 1091/0x0443"),
    (1101, 80, "Standard Init V1.3 Paket 2 Nutzdaten 1101/0x044D"),
    (1181, 90, "Standard Init Extra Paketkopf/Block 1181/0x049D"),
    (1191, 80, "Standard Init V1.3 Paket 3 Nutzdaten 1191/0x04A7"),
    (1271, 90, "Standard Init Parameterblock 1271/0x04F7"),
    (1361, 90, "Standard Init Extra-Block 1361/0x0551"),
    (1451, 90, "Standard Init Extra-Block 1451/0x05AB"),
    (1541, 90, "Standard Init Extra-Block 1541/0x0605"),
    (2001, 90, "Standard Init Statusblock 2001/0x07D1"),
    (2091, 90, "Standard Init Statusblock 2091/0x082B"),
]


class StandardModbusInitReadController:
    """Sequenzieller Controller fuer 'Alle bekannten Register lesen' am Standard-Modbus-Pfad."""
    ...
    def start(self, slave_addr: int, pause_ms: int = 900) -> None:
        owner = self.owner
        if self.active:
            owner._log("STANDARD-MODBUS-INIT läuft bereits; zweiter Start wird ignoriert.")
            return
        self.pause_ms = max(400, int(pause_ms))
        self.timeout_s = max(4.0, float(self.pause_ms) / 1000.0 + 5.0)
        self.queue = [(addr, qty, label, int(slave_addr), 0) for addr, qty, label in STANDARD_MODBUS_INIT_BLOCKS]
        self.retry_items = []
        self.current = None
        self.waiting_since = 0.0
        self.active = True
        ...
        text = ", ".join(f"{addr}/{qty}" for addr, qty, _ in STANDARD_MODBUS_INIT_BLOCKS)
        owner._log(
            f"STANDARD-MODBUS-INIT gestartet: {len(STANDARD_MODBUS_INIT_BLOCKS)} Blöcke, "
    ...
    def step(self) -> None:
        ...
        if not getattr(owner, "connected", False) or not getattr(owner, "worker", None):
            self._finish("STANDARD-MODBUS-INIT abgebrochen: keine aktive Verbindung.")
            return
        ...
        if self.current is not None:
            ...
            try:
                ...
                pass
            owner._log(
                f"STANDARD-MODBUS-INIT TIMEOUT nach {self.timeout_s:.1f}s: "
        ...
        if not self.queue:
            if self.retry_items:
                ...
                self.retry_items = []
                owner._log(f"STANDARD-MODBUS-INIT: starte Retry-Runde fuer {retry_count} Timeout-Block/Blöcke.")
                QTimer.singleShot(max(300, self.pause_ms), self.step)
                return
            self._finish("STANDARD-MODBUS-INIT fertig / alle Blöcke inkl. Retry angefordert.")
            return
        ...
        owner._log(f"STANDARD-MODBUS-INIT Block: {label} ({addr}/{qty}), retry={retry}, verbleibend danach: {total_left}")
